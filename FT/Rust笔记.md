...
引用是不可变的，`&` 符号即是引用，它们允许你使用值，但是不获取所有权
...
如果不同特征拥有同名的方法，类型也有同名的方法，优先调用类型的方法，用::显示调用特征的方法，或者使用完全限定语法\<Dog as Animal>::
...
**引用的生命周期从借用处开始，一直持续到最后一次使用的地方**。减少了很多冲突
...
`'static` 是相当常见的，例如字符串字面值就具有 `'static` 生命周期，还有特征对象。

`&'static` 一个引用必须要活得跟剩下的程序一样久，才能被标注为 `&'static`。`&'static` 生命周期针对的仅仅是引用，而不是持有该引用的变量，对于变量来说，还是要遵循相应的作用域规则。

```rs
let string = "Hello World!";
//“Hello World” 是字符串字面量，因此它的生命周期是 `'static`
//但持有它的变量 `string` 的生命周期就不一样了，它完全取决于变量作用域
//虽然变量被释放，无法再被访问，但是数据依然还会继续存活，可以通过裸指针访问到这个数据
```

`T: 'static` 与 `&'static` 有相同的约束：T必须活得和程序一样久，但是如果是用的\&T，那么不会去检查T
...
FnOnce 该闭包会拿走捕获变量的所有权,该闭包只能调用一次

```rust
fn call_fn_once<F>(func: F)//泛型参数F,代表闭包类型
where
    F: FnOnce() -> i32,  //泛型约束,F必须实现FnOnce()特征,该闭包不接受参数,返回一个i32的值
{
    let result = func();
...
fn main() {
    let x = 5;
    let closure = || {  //定义一个闭包closure    ||表示闭包的参数列表,这里为空
        println!("Captured value: {}", x);
        x * 2
    };
    call_fn_once(closure);
    // 下面这行代码会报错，因为closure已经被移动到call_fn_once中
    // call_fn_once(closure); 
}
```
...
fn main() {
    ...
    // 创建所有权转移的迭代器
    let numbers_owned = vec![1, 2, 3];
    let into_iter = numbers_owned.into_iter();
}
```
...
`for` 循环通过自动调用迭代器上的 `next` 方法，来获取迭代器中的元素。
...
只要迭代器上的某个方法内部调用了next方法,那么这个方法就被称为消费性适配器,因为next会消耗迭代器上的元素

###### 迭代器适配器

会返回一个新的迭代器,是实现链式调用的关键,但是需要一个消费者适配器来收尾,将迭代器转化为一个具体的值

*   collect\
    消费者适配器，使用它可以将一个迭代器中的元素收集到指定类型中
*   zip\
    迭代器适配器，它的作用就是将两个迭代器的内容压缩到一起，形成 `Iterator<Item=(ValueFromA, ValueFromB)>` 这样的新的迭代器
*   `map` 是将迭代器中的值经过映射后

    `filter` 对迭代器中的元素进行过滤，转换成新的值
*   `sum` 是消费者适配器，对迭代器中的所有元素求和
*   `enumerate` 是迭代器适配器,该方法产生一个新的迭代器，其中每个元素均是元组 `(索引，值)`
...
fn shoes_in_size(shoes: Vec<Shoe>, shoe_size: u32) -> Vec<Shoe> {
    shoes.into_iter().filter(|s| s.size == shoe_size).collect()//参数 s 是来自迭代器中的值与外部参数做比较,相等则保留
}
...
~~~rust
   let a: u8 = 10;
   let b: u16 = 1500;
   let b_: u8 = b.try_into();
let b: i16 = 1500;
let b_: u8 = match b.try_into() //会有溢出错误
~~~
...
//相当于Wrapper是vec<i32>的类型别名,然后依靠这个struct去实现Display特性
//newtype就是一个结构体中只有一个元素类型
```
...
集合 `Vec`、`String` 和 `HashMap` 等,**这些底层数据只是保存在堆上，在栈中还存有一个引用类型**,**栈上的引用类型是固定大小的**,所以他们是固定大小

* 切片
* str    let s2: str = "How's it going?"; 报错    let s3: &str = "on?"不报错,是固定大小类型,字符串字面量切片
* 特征对象 使用引用或者Box使用特征对象
...
~~~rust
let s1: Box<str> = Box::new("Hello there!" as str);//不可行,因为不知道长度信息
let s1: Box<str> = "Hello there!".into();//可以,告诉我们要的类型,然后让编译器自动转化
~~~

#### 整数转化为枚举

三方库:num-traits`和`num-derive
...
栈内存从高位地址向下增长，且栈内存是连续分配的,在 Rust 中，`main` 线程的[栈大小是 `8MB`](https://course.rs/compiler/pitfalls/stack-overflow.html)，普通线程是 `2MB`
...
~~~rust
let a = Box::new(3);//堆上分配内存,返回指针
//报错: let b = a + 1; // cannot add `{integer}` to `Box<{integer}>`,在表达式中，我们无法自动隐式地执行 Deref 解引用操作,需要使用*
//println!可以打印,因为隐式调用了解引用
//作用域结束时释放,Box<T> 实现了 Drop 特征
~~~

避免栈上的数据拷贝

 栈上数据转移实际上是数据拷贝了一份,堆上是底层不变,复制了指针,然后旧指针失效
...
~~~rust
enum List {
    Cons(i32, List),//错误,DST导致无限递归
    Nil,
}
enum List {
    Cons(i32, Box<List>),//转换为Sized类型
    Nil,
}
~~~
...
消费Box,强制目标从内存中泄漏.例如，你可以把一个 `String` 类型，变成一个 `'static` 生命周期的 `&str` 类型：
...
为什么不手动标注为\`static标注的 `'static` 只是用来忽悠编译器的，但是超出作用域，一样被释放回收

场景:**你需要一个在运行期初始化的值，但是可以全局有效，也就是和整个程序活得一样久**
...
Drop允许智能指针超出作用域后自动执行数据清除等工作

常规引用是一个指针类型,通过*解引用获取内存地址对应的值
...
智能指针是一个结构体类型不能常规使用*,可以为智能指针结构体实现Deref特征,实现之后就可以通过\*进行解引用
...
}//这时解引用的时候返回的就是&self.0
~~~

#### Drop 释放资源

当使用智能指针来管理锁的时候，你可能希望提前释放这个锁，然后让其它代码能及时获得锁，此时就需要提前去手动 `drop`,drop只是借用了目标值的可变引用

需要自己回收资源的情况:文件描述符,socket等

我们无法为一个类型同时实现 `Copy` 和 `Drop` 特征。因为实现了 `Copy` 特征的类型会被编译器隐式的复制，因此非常难以预测析构函数执行的时间和频率
...
-  let c = Cell::new(String::from("asdf")); 错误,string没有实现copy
...
fn main() {
    ...
    // 创建循环引用
    {
        ...
        node2_mut.next = Some(Rc::clone(&node1));
    }

    // 这里形成了循环引用，导致内存泄漏
    // 无法确定哪个节点应该先被释放
    // 不过Rust的编译器不会阻止这种情况，因为使用了Rc和RefCell
}
~~~

weak可以访问但是没有所有权,不会增加引用计数,不会影响被引用值的释放,可由 `Rc<T>` 调用 `downgrade` 方法转换成 `Weak<T>`,`Weak<T>` 可使用 `upgrade` 方法转换成 `Option<Rc<T>>`，如果资源已经被释放，则 `Option` 的值是 `None`
...
//我们试图同时使用值和值的引用，最终所有权转移和借用一起发生了

~~~

解决办法:

*  使用Option,第一次给值线给None,后面再进行一次给值
* pointer_to_value: *const String   裸指针,unsafe实现
* Pin,所有权无法移动
* ouroboros
...
//使用move拿走所有权
子线程A又创建了子线程B,但是A没有等待子线程B结素,而B是一个死循环,那么将永远不会结束,直至cpu跑满
~~~
...
//打印出 before wait 后增加了一个屏障，目的就是等所有的线程都打印出before wait后，各个线程再继续执行
~~~

线程局部变量  标准库-thread_local

三方库-thread-local,允许每个线程持有值的独立拷贝,该库不仅仅使用了值的拷贝，而且还能自动把多个拷贝汇总到一个迭代器中，最后进行求和，非常好用。
